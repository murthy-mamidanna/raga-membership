...
const SUSTAINER_THRESHOLD =  250.00;
...
// Calculate correct valid_until from qualifying payments only (amount >= SUSTAINER_THRESHOLD)
// Rule: only extend from current expiry if payment is within 60 days of expiry.
// Otherwise treat as a fresh gift: payment_date + 1 year.
function calculateValidUntil(payments: { payment_date: string; amount: string }[]): string | null {
  if (!payments || payments.length === 0) return null;
  const qualifying = [...payments]
    .filter(p => parseFloat(p.amount) >= SUSTAINER_THRESHOLD)
...
// Build notes for any special renewals detected (qualifying payments only)
function buildRenewalNotes(payments: { payment_date: string; amount: string }[]): string {
...
serve(async (req) => {
  ...
  try {
    ...
    // 3. Calculate correct valid_until using qualifying payments only (>= $250)
    const calculatedValidUntil = calculateValidUntil(payments);

    // No qualifying payments >= $250 — found in DB but not a sustainer
